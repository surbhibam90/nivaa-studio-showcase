...
export function HeroVideo() {
  ...
  return <div className="hero-media group" onMouseEnter={start} onMouseLeave={stop} onClick={()=>active?stop():start} role="button" tabIndex={0} onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();active?stop():start()}}} aria-label={active?"Stop living room film":"Play living room film"}>
    ...
    <video ref={video} src={videoAsset.url} poster={posterAsset.url} muted playsInline preload="metadata" className={`absolute inset-0 h-full w-full object-cover transition-opacity duration-700 ${active?"opacity-100":"opacity-0"}`} onEnded={stop}/>
    <span className="absolute bottom-6 right-6 grid size-12 place-items-center rounded-full border border-background/65 bg-background/80 backdrop-blur-md"><Play className="size-4 fill-current"/></span>
    <span className="absolute bottom-8 left-8 hidden text-[.58rem] uppercase tracking-[.25em] text-background drop-shadow-md md:block">Hover to step inside</span>
  </div>;
}
